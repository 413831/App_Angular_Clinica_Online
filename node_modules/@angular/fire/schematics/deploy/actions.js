...
const deployToHosting = (firebaseTools, context, workspaceRoot, preview) => {
    if (preview) {
        const port = 5000;
        setTimeout(() => {
            child_process_1.execSync(`open http://localhost:${port} || true`);
        }, 1500);
        return firebaseTools.serve({ port, targets: ["hosting"] }).then(() => require('inquirer').prompt({
            type: 'confirm',
            name: 'deployProject',
...
const getPackageJson = (context, workspaceRoot) => {
    ...
    const npmList = child_process_1.execSync('npm ls || true').toString();
...
exports.deployToFunction = (firebaseTools, context, workspaceRoot, project, preview, fsHost = defaultFsHost) => __awaiter(void 0, void 0, void 0, function* () {
    if (!semver_1.satisfies(process.versions.node, getVersionRange(functions_templates_1.NodeVersion))) {
        context.logger.warn(`⚠️ Your Node.js version (${process.versions.node}) does not match the Firebase Functions runtime (${functions_templates_1.NodeVersion}).`);
    }
    if (!project ||
        !project.build ||
    ...
    if (preview) {
        ...
        return firebaseTools.serve({ port, targets: ["hosting", "functions"] }).then(() => require('inquirer').prompt({
            type: 'confirm',
            name: 'deployProject',
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            if (ssr) {
                ...
                success = yield deployToHosting(firebaseTools, context, context.workspaceRoot, preview);
            }
        }
        catch (e) {
